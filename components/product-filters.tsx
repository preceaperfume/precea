...
export function ProductFilters({ initialCollection }: { initialCollection?: string }) {
  const [query, setQuery] = useState("");
  const [family, setFamily] = useState("All");
  const [sort, setSort] = useState("Featured");
  const [collection, setCollection] = useState(initialCollection ?? "All");
  const collections = ["All", ...Array.from(new Set(perfumes.map((product) => product.collection)))];
